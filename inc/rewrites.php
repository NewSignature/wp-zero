 ...
 *
 */
function zero_add_rewrites( $content )
{
  global $wp_rewrite;
  $zero_non_wp_rules        = array(
    'assets/(.*)'  => THEME_PATH . '/assets/$1',
    'plugins/(.*)' => RELATIVE_PLUGIN_PATH . '/$1',
    'cache/(.*)'   => 'wp-content/cache' . '/$1'
  );
  $wp_rewrite->non_wp_rules = array_merge( $wp_rewrite->non_wp_rules, $zero_non_wp_rules );
  return $content;
}

function zero_clean_urls( $content )
{
...
if ( ! is_multisite() && ! is_child_theme() ) {
  if ( current_theme_supports( 'rewrites' ) ) {
    add_action( 'generate_rewrite_rules', 'zero_add_rewrites' );
  }
  ...
  if ( ! is_admin() && current_theme_supports( 'rewrites' ) ) {
    ...
    add_filters( $tags, 'zero_clean_urls' );
  }
}
